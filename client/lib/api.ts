...
export const checkServerReadiness = async (): Promise<boolean> => {
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Server readiness check failed with status ${response.status}`,
      );
    }
...
export const getSkillBridgeData = async (
  ...
  try {
    const response = await fetch(
      `${API_BASE_URL}${API_PREFIX}/recommend-courses`,
      {
        method: "POST",
        headers: {
...
export const extractSkillComparisonData = (
  response: SkillBridgeResponse,
): SkillComparisonData => {
  ...
  const matchedSkills = response.job_skills.filter(
    (skill) => !response.skill_gap.includes(skill),
  );
